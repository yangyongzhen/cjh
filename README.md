...
> 📋 **完整功能清单**（v1.3.15）→ [docs/cjh功能清单.md](docs/cjh功能清单.md) —— Agent 主循环 / 13+ 内置工具 / 10 主题 / 插件系统与信任链 / MCP / 双平台 / 版本历史的完整盘点，随版本持续更新。
...
| **工程质量** | 325 单测 + 15 PTY 场景（49 断言），测试真实抓出 10+ 潜伏 bug（含 edit 工具"此前从未真正工作过"）；性能优化有实测数据（prompt 峰值 42.9K→9.4K、单轮 5-22s→2-5s）；CI 门禁（测试全绿、先红后绿）强制执行 |

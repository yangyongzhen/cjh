...
> 📋 **完整功能清单**（持续更新至 v1.4.0）→ [docs/cjh功能清单.md](docs/cjh功能清单.md) —— Agent 主循环 / 15 内置工具 / 10 主题 / 插件系统与信任链 / MCP / 双平台 / 版本历史的完整盘点，随版本持续更新。
...
**397 个单元测试全绿**（根包 + `libs/cjterm` 库包 93 例，`./scripts/test.sh` 一键运行、自动切动态链接配置；库包测试需 `cd libs/cjterm && cjpm test`）+ **61 个 PTY 集成测试场景**（`python3 scripts/tui_pty_test.py`，伪终端驱动真实 TUI），覆盖全部 15 个内置工具 + Agent 核心 + TUI 渲染/事件/思考交织 + 基础设施：

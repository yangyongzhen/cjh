...
- 思考渲染性能门禁（第四十二轮追加）：`cd libs/cjterm && cjpm test` = **PASSED: 76, FAILED: 0**（67→76，新增 slack 2 例 + Rune 缓存 3 例 + 埋点 4 例）；`./scripts/test.sh` = **TOTAL: 367, FAILED: 0**（exit 0）；`cjpm build` exit 0；`--mock` exit 0（32 行、命中 `FINAL-DONE`）；`tui_pty_test.py` = **61 通过 / 0 失败**

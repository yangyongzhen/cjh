...
| 采样 | 改前 avg / maxFoldUs | 改后 avg / maxFoldUs |
|---|---|---|
| renders=240 | 3069 / 10752 | 232 / 4783 |
| renders=360 | 4051 / 17337 | 288 / 4783 |
| renders=480 | 4459 / 17337 | 311 / 4783 |
...
- **思考渲染性能优化（第四十二轮追加，版本号沿用 v1.3.26 不升）**：① 单轮截断**摊薄**——`ThinkingLog.applyRoundCap` 由"每次追加超限即重截"改为留 1/4 上限的 slack，攒够才截回上限（截断是 O(上限)，触发频次降为 1/4，语义不变；新不变量：轮长 ≤ 上限 + 上限/4）；② 折行**去逐 rune 拼接**（`foldThinkingBlock` 改 `StringBuilder` + 行尾一次 `toString()`），并给**活跃轮加 Rune 数组缓存**（`ThinkingBlock.runeArray()`，在追加/改文本与轮结束时失效，省掉每帧 8192 Rune ≈32KB 的重切分配）；③ 新增 **opt-in 渲染性能埋点** `libs/cjterm/src/perf.cj`（`CJH_TRACE_PERF=1` 才开，落 `~/.cjh/cjh_perf.log`，每 60 次渲染或 2s 一行 `renders=/folds=/avgFoldUs=/maxFoldUs=/lastFoldUs=/lastLines=/lastActiveRunes=`；**关闭时零开销**——`beginFold()` 连时钟都不读）；④ `libs/cjterm/src/trace.cj` 把 `defaultPathFor` 泛化为 `defaultPathNamed`、`ensureParentDir/writeLine/stampUs` 提升为包内可见，供埋点复用同一套开关解析与落盘原语（不复制 fopen/fputs）
- `libs/cjllm/src/mock.cj` 新增两个 additive、默认关的旋钮：`CJH_MOCK_REASONING_CHARS=N`（把 mock 思考流垫到 ≥N Rune，用于复现长思考的折行与单轮中间截断路径）、`CJH_MOCK_REASONING_DELAY_MS=ms`（块间延迟，让"进行中"的长轮持续多帧，而非被 5ms 主循环合并成一次渲染）——为"先量再改"提供可复现的长思考输入；纯函数 `reasoningPadChunks` 配 `libs/cjllm/src/mock_test.cj`（4 例：默认不垫 / 垫到目标 / 溢出受控 / 多字节不切碎）
- **思考渲染成本实测**（同上端口径）：展开态长思考单帧折行**平均 4.5ms、典型 5~10ms、峰值 17.3ms**（折叠态 `maxFoldUs=3`），每帧折叠最多 **475 行**而可见内容区约 30 行——**约九成折行在屏幕外**；据此判定「头段冻结 + 尾段增量折行」值得做（待实施），详见 §3.34
- `libs/cjterm/src/perf_test.cj`（4 例：默认关闭零副作用 / 虚拟时钟计数与统计串 / 计数到点落盘 / 间隔到点落盘）；`libs/cjterm/src/thinking_log_test.cj` 增 5 例（slack 摊薄阈值语义 + 小上限 Rune 边界；Rune 缓存追加失效 / 轮结束释放 / 截断失效）
...
- 思考渲染性能门禁（第四十二轮追加）：`cd libs/cjterm && cjpm test` = **PASSED: 76, FAILED: 0**（67→76，新增 slack 2 例 + Rune 缓存 3 例 + 埋点 4 例）；`./scripts/test.sh` = **TOTAL: 367, FAILED: 0**（exit 0）；`cjpm build` exit 0；`--mock` exit 0（32 行、命中 `FINAL-DONE`）；`tui_pty_test.py` = **61 通过 / 0 失败**；`cd libs/cjllm && cjpm test` = **38/38**（含新增 4 例垫片用例）
